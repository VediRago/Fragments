# Development Process

## Early Versions

The earliest files are useful because they show where the project started.

Some early versions were more focused on atmosphere, worldbuilding, and direct explanation. They had more description and more visible setup.

Over time, I became more interested in the people inside those scenes.

The early material is not there to show perfection. It is there to show development.

It shows what changed.

---

## From End-State Images To Structure

Fragments began from end-state images and emotional anchors before the full architecture existed.

Important early anchors included:

- the hospital girl
- Lucreta
- the silhouette / completed-self image
- collapse and cycle-zero imagery

At first, these images felt connected before I could fully explain why.

The deeper structure developed as a way to understand why those images belonged together, not as something imposed before the scenes existed.

The project gradually moved away from explanation-driven concepts and toward a pattern-driven story where meaning is carried through:

- repeated behaviors
- visual residue
- delayed recognition
- perspective limitation
- emotional continuity

---

## From Explanation to Evidence

One of the biggest changes was moving away from explaining the hidden structure directly.

Instead of telling the reader what everything means, I became more interested in showing evidence through repeated behavior, familiar images returning later, small habits that carry emotion, scenes that feel different after looking back, and endings that show consequences instead of explaining them.

This made the story feel less like it was giving answers and more like it was creating realizations.

The goal became less about showing every connection and more about choosing the clearest evidence.

The deeper reference material can hold everything.

The public-facing material should show only the strongest examples of how the story works.

---

## Prose Progression

The prose shift across Fragments became part of the development process.

Lucreta and Unity worked like opposite benchmarks:

- Lucreta: baseline reality, stable description, physical grounding
- Unity: endpoint, perception collapse, images, colors, and residue over environment

The intermediate acts gradually move the reader from stable lived reality toward continuity-driven perception.

The prose does not change simply because of inconsistency.

It changes because reader perception changes.

---

## Connection Was Already There

One of the most useful lessons came while revising Sera's final scene.

I thought the scene needed more connection to work.

But after removing parts I thought were necessary, I felt more engaged with the moment, not less.

The connection was already there.

That changed how I approached later scenes. I started paying more attention to what was already being carried by previous interactions, instead of adding more explanation.

---

## Complexity Has To Return

Fragments can easily become complex.

There are many characters, hidden connections, repeated images, and structural rules underneath the story.

While working on it, I often found myself exploring increasingly complex ideas. What helped me stay grounded was making sure those ideas eventually led back to the characters and their experiences.

If they did not, they did not belong in the story.

The structures matter because they hold the project together.

But the reader experiences people first.

---

## What I Am Still Learning

This project made me notice that meaning can be felt before it is understood.

It made me notice that explanation is not always the best way to communicate an idea.

It made me notice that small details can carry more weight than I expected when the connection is already there.

It also made me notice that revision is not only about adding more. Sometimes the work improves when I remove what is getting in the way.

The deeper reference material remains in the repository as part of the working process, but it is not the recommended starting point.
